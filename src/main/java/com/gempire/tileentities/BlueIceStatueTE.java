...
public class BlueIceStatueTE extends BlockEntity implements GeoBlockEntity {

    int timer;
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    public BlueIceStatueTE(BlockPos pos, BlockState state) {
        super(ModTE.BLUE_ICE_STATUE_TE.get(), pos, state);
        timer = 2400;
    }
    @Override
    ...
    public static <T extends BlockEntity> void tick(Level level, BlockPos pos, BlockState state, T be) {
        if (be instanceof BlueIceStatueTE) {
            if (((BlueIceStatueTE) be).timer != 0) {
                ((BlueIceStatueTE) be).timer--;
