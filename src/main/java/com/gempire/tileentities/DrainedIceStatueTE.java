...
public class DrainedIceStatueTE extends BlockEntity implements GeoBlockEntity {

    int timer;
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    public DrainedIceStatueTE(BlockPos pos, BlockState state) {
        super(ModTE.DRAINED_ICE_STATUE_TE.get(), pos, state);
        timer = 2400;
    }
    ...
    public static <T extends BlockEntity> void tick(Level level, BlockPos pos, BlockState state, T be) {
        if (be instanceof DrainedIceStatueTE) {
            if (((DrainedIceStatueTE) be).timer != 0) {
                ((DrainedIceStatueTE) be).timer--;
