...
public class IceStatueTE extends BlockEntity implements GeoBlockEntity {

    int timer;
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    public IceStatueTE(BlockPos pos, BlockState state) {
        super(ModTE.ICE_STATUE_TE.get(), pos, state);
        timer = 2400;
    }
    ...
    public static <T extends BlockEntity> void tick(Level level, BlockPos pos, BlockState state, T be) {
        if (be instanceof IceStatueTE) {
            if (((IceStatueTE) be).timer != 0) {
                ((IceStatueTE) be).timer--;
